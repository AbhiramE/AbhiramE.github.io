---
title: "A Shot at Building a Secure Claw"
date: 2026-04-21
draft: false
tags: ["ai", "agents", "security", "system-design"]
description: "Designing a Claw that I'd trust anywhere near my real credentials."
ShowToc: true
TocOpen: false
---
...
The Claw ecosystem already has pretty solid answers to the "safe agent harness" question. [OpenClaw](https://github.com/openclaw/openclaw) is the OG: broad, batteries-included, and covering pretty much everything under the sun. [NanoClaw](https://github.com/qwibitai/nanoclaw) goes the other way: one process, a handful of files, and customisation by editing code instead of wrangling config. [IronClaw](https://github.com/nearai/ironclaw) is the security-first version — sandboxed tools, credentials injected at the boundary, leak checks on the way out.
...
<figure class="align-center" style="margin: 1rem auto; max-width: 75%;">
  <img src="/images/claw-redux-arch.png" alt="Claw architecture diagram: WhatsApp to Gateway to isolated Agent container." style="display:block; margin: 0 auto; width: 100%; height: auto;">
  <figcaption><p><em>The agent is the untrusted user. A deterministic gateway holds the credentials, gates the writes, and logs every call.</em></p></figcaption>
</figure>
...
The next pressure on this design is going to come from richer tools. Browser use, long-running code execution, agents wrapping other agents, all of that wants a bigger hole than a single tool call. And the sandbox story itself isn't solved. For example this recent bypass of AWS AgentCore's sandbox network isolation mode using DNS tunneling in [Cracks in the Bedrock: Escaping the AWS AgentCore Sandbox](https://unit42.paloaltonetworks.com/bypass-of-aws-sandbox-network-isolation-mode/). That is a pretty good reminder that "sandboxed" is not a magic word. It is a claim you keep testing.
